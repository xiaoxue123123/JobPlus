...
public class MonkeyLearnClient {
    ...
    private static final String AUTH_TOKEN = "21bb42cc2ef278a78abe729f06b1596c3d273a4a";

    public List<Set<String>> extract(List<String> articles) {
        ObjectMapper mapper = new ObjectMapper();
        //used apache http library
        CloseableHttpClient httpClient = HttpClients.createDefault();

        HttpPost request = new HttpPost(EXTRACT_URL);

        // 对应postman里的request header，Token后面注意有空格
        request.setHeader("Content-type", "application/json");
        request.setHeader("Authorization", "Token " + AUTH_TOKEN);

        // 把article map 成JSON
        ExtractRequestBody body = new ExtractRequestBody(articles, 5);

        String jsonBody;

        try {
            // convert ExtractRequestBody class里面的field 成String 
...
//            System.out.println("jsonBody" + jsonBody);
        ...
        // 处理execute之后的结果
        // anonymous function: response 是传进去的 parameter
